...
A GitHub Action that validates that third-party actions in your workflows and repository-root action metadata use immutable releases, enhancing supply chain security.
...
This action scans your workflow files and repository-root `action.yml` or `action.yaml`, then validates that third-party actions are using [immutable releases](https://docs.github.com/en/repositories/releasing-projects-on-github/managing-releases-in-a-repository#creating-immutable-releases). This prevents supply chain attacks where a release could be modified after you've started using it. First-party actions (`actions/*`, `github/*`, and `octokit/*`) are excluded from checks by default, but can be included via the `include-first-party` input.

The scan covers:

- **Step-level actions** (`steps[].uses`)
- **Job-level reusable workflows** (`jobs.<id>.uses`)
- **Repository-root action metadata** — automatically scans `action.yml` or `action.yaml` composite steps, even when no workflow references the action
...
The action generates a report organized by scanned workflow or root action metadata file, making it easy to identify which entrypoints need attention:
...
### Check all workflows and root action metadata (default)
...
Bare filenames exclude matching workflow names everywhere, including during recursive traversal of reusable workflows.
...
Patterns containing `/` match the full workflow path without the `@ref`, which lets you exclude one remote reusable workflow without excluding identically named workflows in other repositories:

```yaml
- name: Ensure immutable actions
  uses: joshjohanning/ensure-immutable-actions@v2
  with:
    exclude-workflows: 'some-org/some-repo/.github/workflows/ci.yml'
```
...
1. **Scans Entrypoints**: Reads all workflow files (or specified ones) from `.github/workflows/` and composite steps from repository-root `action.yml` or `action.yaml`
